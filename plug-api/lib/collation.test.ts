...
test("compareCollated: disabled collation falls back to raw codepoint order", () => {
  ...
  expect(compareCollated("Zeta", "apple", { enabled: false }, collator))
    .toBeLessThan(0);
  expect(compareCollated("Zeta", "apple", undefined, collator))
    .toBeLessThan(0);
});
...
test("compareCollated: enabled collation sorts alphabetically, not by codepoint", () => {
  ...
  expect(compareCollated("Zeta", "apple", collation, collator))
    .toBeGreaterThan(0);
  expect(compareCollated("apple", "Zeta", collation, collator))
    .toBeLessThan(0);
});
...
test("compareCollated: equal values return 0", () => {
  const collator = Intl.Collator(undefined, undefined);
  expect(compareCollated("same", "same", { enabled: true }, collator))
    .toBe(0);
