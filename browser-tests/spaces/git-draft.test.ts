import type { GitDraft } from "../../client/spaces_ui/types.ts";
...
test("draft editing invalidates a late check and Cancel preserves the active connection", async ({
  ...
  await page.goto(`${origin}/.spaces/sample/git`);
  ...
  await expect(page).toHaveURL(`${origin}/.spaces/sample?section=revisions`);
