...
test("custom hostname origins use the Primary URL scheme without inheriting its port", () => {
  expect(
    customHostOrigin(
      "notes.example.com",
      resolvePublicOrigin(
        "https://manage.example.com:8443/admin",
        "http://localhost:3000",
      ),
    ),
...
test("hostOptions folds explicit Primary-host bindings into Primary occupancy", () => {
  const spaces = [
    ...
    space("explicit", {
      host: "MANAGER.EXAMPLE.TEST.:8443",
      prefix: "/wiki",
    }),
    space("root", { host: "manager.example.test:8443" }),
    space("team", { host: "team.example.test", prefix: "/work" }),
  ];
  const result = bindingHostOptions(
    spaces,
    "manager.example.test:8443",
    "implicit",
  );
