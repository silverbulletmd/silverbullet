...
// `DASHBOARD_BASE` is computed at module load from `document.baseURI`, so the
...
const HERE = "http://localhost:3000/.dashboard/users";
...
test("another Dashboard screen is handled in place", async () => {
  const { isDashboardUrl } = await load(BASE, HERE);
  expect(
    isDashboardUrl(new URL("http://localhost:3000/.dashboard/users/bob")),
  ).toBe(true);
});

test("the Dashboard root itself is handled in place", async () => {
  const { isDashboardUrl } = await load(BASE, HERE);
  expect(isDashboardUrl(new URL("http://localhost:3000/.dashboard"))).toBe(
    true,
  );
  expect(isDashboardUrl(new URL("http://localhost:3000/.dashboard/"))).toBe(
    true,
...
test("a link to a space itself is left to the browser", async () => {
  ...
  const { isDashboardUrl } = await load(BASE, HERE);
  expect(isDashboardUrl(new URL("http://localhost:3000/notes/"))).toBe(false);
  expect(isDashboardUrl(new URL("http://localhost:3000/"))).toBe(false);
});

test("a path merely starting with the base's text is not a screen", async () => {
  const { isDashboardUrl } = await load(BASE, HERE);
  expect(
    isDashboardUrl(new URL("http://localhost:3000/.dashboard-other/x")),
  ).toBe(false);
});
...
test("another origin is left to the browser", async () => {
  const { isDashboardUrl } = await load(BASE, HERE);
  expect(
    isDashboardUrl(new URL("https://evil.example.com/.dashboard/users")),
  ).toBe(false);
});
...
test("an empty base intercepts nothing", async () => {
  ...
  const { isDashboardUrl } = await load(
    "http://localhost:3000/",
    "http://localhost:3000/",
  );
  expect(isDashboardUrl(new URL("http://localhost:3000/anything"))).toBe(false);
});
...
test("a plain left-click on an in-app link is intercepted", async () => {
  ...
  expect(
    shouldIntercept(
      click(),
      anchor("http://localhost:3000/.dashboard/users/bob"),
    ),
  ).toBe(true);
});
...
test("a download link is left to the browser", async () => {
  ...
  expect(
    shouldIntercept(
      click(),
      anchor("http://localhost:3000/.dashboard/export", {
        hasAttribute: () => true,
      }),
    ),
  ).toBe(false);
