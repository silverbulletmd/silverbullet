...
test("drops entries that are not tables", () => {
  ...
  expect(
    normalizeViewDefaults({ "std.toc": "page-top", ok: { open: true } }),
...
test("keeps an entry's good fields when only some are wrong-typed", () => {
  const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
  expect(
    normalizeViewDefaults({ "std.toc": { dock: "rhs", width: "wide" } }),
...
test("clamps width to the panel bounds", () => {
  expect(
    normalizeViewDefaults({ a: { width: 40 }, b: { width: 5000 } }),
...
test("legacy tables contribute nothing for non-string values", () => {
  expect(mergeLegacyDocks({}, { a: 3, b: "rhs" })).toEqual({
    b: { dock: "rhs" },
  });
});
