...
test.describe("system.reboot()", () => {
	...
	test("out-of-band create: a new on-disk page is indexed after reboot", async ({ sbServer, page }) => {
		await gotoSilverBulletPage(page, sbServer);

		// The page does not exist yet.
		expect(
			await query(page, `from index.tag "page" where name == "OutOfBand" select name`),
		).toEqual([]);
	...
	test("out-of-band modify: index reflects new on-disk content after reboot", async ({ sbServer, page }) => {
		...
		expect(
			await query(page, `from index.tag "page" where name == "Note" select name`),
		).toEqual(["Note"]);
		expect(await query(page, `from index.tag "rebootmodify" select name`)).toEqual([]);
		...
		expect(await query(page, `from index.tag "rebootmodify" select name`)).toEqual(["Note"]);
	});

	test("out-of-band delete: removed file's objects are gone after reboot", async ({ sbServer, page }) => {
		const tempPath = join(sbServer.spaceDir, "Temp.md");
		await writeFile(tempPath, "# Temp\ntemporary page\n");
		await gotoSilverBulletPage(page, sbServer);

		expect(
			await query(page, `from index.tag "page" where name == "Temp" select name`),
		).toEqual(["Temp"]);
	...
	test("drain-before-return: query immediately after reboot sees the change (no sleep)", async ({ sbServer, page }) => {
		await gotoSilverBulletPage(page, sbServer);
		...
		expect(
			await query(page, `from index.tag "page" where name == "Racy" select name`),
		).toEqual([]);
	...
	test("script edits applied: a new Library Space Lua function is live after reboot", async ({ sbServer, page }) => {
		await gotoSilverBulletPage(page, sbServer);
	...
	test("error surfacing: a Lua load error appears in the console after reboot", async ({ sbServer, page }) => {
	...
	test("no-op: reboot with nothing changed completes promptly and keeps the index intact", async ({ sbServer, page }) => {
		await gotoSilverBulletPage(page, sbServer);
		...
		expect(
			await query(page, `from index.tag "page" where name == "index" select name`),
		).toEqual(["index"]);
	});

	test("in-flight concurrency: reboot converges while file operations are in flight", async ({ sbServer, page }) => {
		await gotoSilverBulletPage(page, sbServer);
		await writeFile(join(sbServer.spaceDir, "Concurrent.md"), "# Concurrent\nconcurrent check\n");
		...
		expect(
			await query(page, `from index.tag "page" where name == "Concurrent" select name`),
		).toEqual(["Concurrent"]);
	});
});
