 ...
 */
async function waitForWidgetsReady(
  page: Page,
  timeoutMs = 15_000,
): Promise<void> {
  const deadline = Date.now() + timeoutMs;
...
test.describe("index version upgrade (service worker disabled)", () => {
  ...
  test("stale index version triggers a real reindex on next boot", async ({
    sbServer,
    page,
  }) => {
    ...
    expect(consoleState.messages.some((m) => m.includes(REINDEX_LOG))).toBe(
      false,
    );
...
test.describe("index version upgrade (service worker enabled)", () => {
  ...
  test("stale index version triggers a real reindex after sync + version match", async ({
    sbServer,
    page,
  }) => {
    const consoleState = captureConsole(page);
