...
The syntax for SLIQ is `query[[my query]]`. In regular Lua `[[my query]]` is just another way of writing `"my query"` (it is an alternative string syntax). Function calls that only take a string argument can omit parentheses, therefore `query[[my query]]` is equivalent to `query("my query")`.
...
Unlike in SQL where clauses must be written in a particular order, SLIQ clauses can be written in any order, and the only mandatory clause is the `from` clause.
...
These can be combined. See [[Space Lua/Integrated Query/Aggregating]] for details.

SLIQ operates on any Lua collection.
...
See [[Space Lua/Integrated Query/Grouping]] for detailed examples.
