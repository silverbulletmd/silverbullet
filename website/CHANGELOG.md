...
* Fix: transcluded pages (e.g. `![[Some Page]]`) no longer render excessive blank space between block elements like headings, lists and queries (caused by whitespace between blocks becoming spurious `<br>` tags). Heading text is also now trimmed so `# Heading` renders as `<h1>Heading</h1>` rather than `<h1> Heading</h1>`.
