//! The single logged-in web surface, served at `/.dashboard`. Any valid account
...
async fn handle_login(
    ...
    let (jwt, secs) = match state.login.issue_session(&body.username, body.remember_me) {
        Ok(value) => value,
        Err(error) => {
            tracing::error!("failed to mint dashboard session JWT: {error}");
...
async fn handle_logout(State(state): State<Arc<DashboardState>>, headers: HeaderMap) -> Response {
...
fn current_username(state: &DashboardState, headers: &HeaderMap) -> Option<String> {
    let name = scoped_auth_cookie_name(&request_host(headers), "");
...
async fn handle_session(State(state): State<Arc<DashboardState>>, headers: HeaderMap) -> Response {
...
async fn handle_list(State(state): State<Arc<DashboardState>>, headers: HeaderMap) -> Response {
    let Some(username) = current_username(&state, &headers) else {
        return (StatusCode::UNAUTHORIZED, "Unauthorized").into_response();
    };
    let admin = state.users.is_admin(&username);
...
pub fn build_dashboard_router(state: Arc<DashboardState>, admin_api: Router) -> Router {
    Router::new()
...
mod tests {
    ...
    fn setup_with(session: SessionPolicy) -> (tempfile::TempDir, Router) {
        ...
        bundle
            .write_file(".client/dashboard.html", b"DASHBOARD-SHELL", None)
            .unwrap();
        bundle
    ...
    /// distinction is load-bearing for the client: `/.dashboard` is the landing
    ...
    /// priority that `build_dashboard_router` relies on. If axum ever stopped
