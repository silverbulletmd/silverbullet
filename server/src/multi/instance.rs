...
///   already-populated space costs O(1), not a full listing, on every boot.
///   Uses `DiskSpacePrimitives::write_file` for the actual write so nested
///   index pages (e.g. `notes/index`) get their parent directories created
///   for free.
