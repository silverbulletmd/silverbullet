...
---
A **recipient** is an [[Concepts/Identity]] you address. When mentioned with `@name`, it is indexed to be queried, and clicking it in the editor will open the "Mentions Inbox". -- @zef

See [[Concepts/At-Mention]] for the `@name` syntax, and [[Concepts/Identity]] for where names come from (accounts, `identity.define`, or simply being mentioned).

# The Mention Inbox
The Mention Inbox is a [[Features/Navigator]] view listing open `@mention`s, grouped by page. It opens in the right sidebar by default, and its dock menu can move it to a modal. Open it with `Navigate: Mentions`, or by clicking any mention.
...
A mention only ever means one thing: *this is for you*. Marking text as *written by* someone instead uses a slightly different syntax, see [[Concepts/Authorship]].

# Querying mentions
At-mentions are indexed like any other [[Concepts/Object Index|relation]], with `kind == "at-mention"`. `to` is always the `@name` identifier (`toTag == "identity"`) and `alias` holds the name as typed. Build your own view over them with SLIQ, for example @pete.smith:
