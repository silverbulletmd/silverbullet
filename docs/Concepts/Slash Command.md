...
# Slash templates
Most slash commands are implemented as [[Features/Slash Template]] — pages tagged with `#meta/template/slash` whose content is inserted at the cursor. The standard library includes slash templates for:
...
1. **Slash templates** (recommended): Create a page tagged `#meta/template/slash`. See [[Features/Slash Template]] for details.
