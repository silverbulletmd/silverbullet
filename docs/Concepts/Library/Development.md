...
Put all functionality you like in a [[Concepts/Meta Page|Meta Page]] somewhere under `Library/`. For name spacing purposes it’s good form to put your (Github) username in the path as well, so create, for instance `Library/myuser/My Library`.

Then, decorate your library page with some [[Concepts/Frontmatter]]. Here is an minimal example library that defines a new “Hello world” command:
...
# Share the library
At this stage you can put your library file anywhere you like with a URL. However, for convenience, it’s good to use [[Features/Share]] support for publishing.
...
You’ll notice that a few [[Features/Share]] related frontmatter keys were set in your local copy. You can now make further changes to your library and run the ${widgets.commandButton "Share: Page"} command again. After entering another commit message your page will be pushed again. This is how you publish new versions of your library for other people to use.
...
# Create a repository
If you develop a few libraries, it may be good to make them more discoverable. To do so, you can group them in a [[Concepts/Repository|Repository]].
...
Now share this repo page using [[Features/Share]] to the same github repo (or any other, it doesn’t really matter), and call it `REPO.md` (by convention).
