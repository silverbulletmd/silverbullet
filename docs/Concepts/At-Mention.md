...
---
An **at-mention** is the `@name` span you type in text. It refers to an [[Concepts/Identity]], and it has two directions: written plainly it *addresses* that identity, making it a [[Concepts/Recipient]], or when used as a signature it *credits* that identity instead, see [[Concepts/Authorship]].
...
**Any name is a valid mention.** Nothing needs declaring up front: mentioning @sales makes `sales` a known identity. See [[Concepts/Identity]] for more info.