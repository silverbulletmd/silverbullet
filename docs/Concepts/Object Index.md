...
---
The Object Index stores [[Object|Objects]] found in your [[Concepts/Space]].

You interact with it in a few ways:
* Indirectly via various other SilverBullet features such as [[Features/Page Picker]], [[Features/Meta Picker]], [[Concepts/Linked Mention]] etc.
...
* `index.contentPages(tag?)` — returns only content pages, **excluding [[Concepts/Meta Page|Meta Pages]]** (pages tagged `meta` or `meta/*`).
* `index.aspiringPages()` — returns all [[Concepts/Aspiring Page]] (pages linked to but not yet created). Useful for finding broken or forward-references.
