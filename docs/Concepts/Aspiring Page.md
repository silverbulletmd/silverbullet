...
An aspiring page is a [[Concepts/Page|page]] that does not yet exist, but is already linked to.

Aspiring pages appear in the [[Features/Page Picker]] (with a `Create` chip) as well as in auto complete when creating [[Concepts/Link]].

# Finding dangling links
Every `[[link]]` to a non-existent page produces an `aspiring-page` object in the [[Concepts/Object Index]], so you can query them to audit broken or forward-references:
