...
- docs/Recipient.md
---
An **author** is an [[Concepts/Identity]] you credit. A signature is an [[Concepts/At-Mention]] used the other way around: an `@mention` **addresses** someone, while a signature **credits** someone. The same identities serve both — anyone you can mention, you can also sign as, including yourself.
...
# What signing does
A signature is not a [[Concepts/Recipient]] [[Concepts/At-Mention]]. It does not enter somebody’s Mention Inbox. Instead, it **attributes** the block it ends: every `@mention` inside that block is stamped with a `by` value naming who signed it, and the signature emits its own `authored` relation.
