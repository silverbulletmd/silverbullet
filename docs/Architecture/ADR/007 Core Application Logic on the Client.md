...
* **The server became trivial and swappable.** Because it only moves bytes, the whole backend could be re-implemented in another language without changing application behaviour — what made subsequent server migrations cheap (see [[Architecture/ADR/010 Rust Backend]]).
