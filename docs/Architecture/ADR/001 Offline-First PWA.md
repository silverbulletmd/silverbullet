...
related:
  - "[[Architecture/ADR/002 Sync Engine]]"
...
* **Keeps the server “dumb”.** The service worker speaking the HTTP API locally is exactly what lets the server stay a thin file store ([[Architecture/ADR/010 Rust Backend]]).

## Negative / trade-offs
* **Full local copy + per-client index.** Every browser/device syncs the space and builds its own IndexedDB index — storage plus a cold-start (re)index cost (see [[Architecture/ADR/007 Core Application Logic on the Client]]). Occasionally a storage-schema change forces a full resync + reindex on all devices.
