...
owner: "[[Zef Hemel]]"
dependsOn:
  - "[[Architecture/ADR/005 Space Lua]]"
  - "[[Architecture/ADR/003 Indexed Object Graph]]"
references:
...
* **Still one language.** SLIQ is sugar *within* Lua, not a separate DSL — it preserves the "one language, no seams" win of [[Architecture/ADR/005 Space Lua]] (a query can `select` a template call, use Lua helpers, etc.).
...
## Negative / trade-offs
* **A query dialect to parse and maintain.** SLIQ is a syntax extension inside the custom Lua implementation — only feasible *because* the interpreter is our own ([[Architecture/ADR/005 Space Lua]]).
...
* **A separate standalone query language (as in v1).** Rejected: reintroduces a second language and the seams between query, template, and script that [[Architecture/ADR/005 Space Lua]] set out to remove.
