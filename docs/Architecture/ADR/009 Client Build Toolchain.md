...
---
# Context
Since [[Architecture/ADR/007 Core Application Logic on the Client|v2]] the client is where all the logic lives, and it is built with TypeScript + ESBuild. For years that toolchain ran on **Deno**, chosen early when Deno's sandboxed-worker permissions also suited running [[Plugs|plugs]] on the server (see [[Architecture/ADR/010 Rust Backend]]).
