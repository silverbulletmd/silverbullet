...
owner: "[[Zef Hemel]]"
dependsOn:
  - "[[Architecture/ADR/001 Offline-First PWA]]"
related:
...
* **Keeps the server dumb.** File-level sync needs only list/read/write/delete, so the server stays a thin file store ([[Architecture/ADR/010 Rust Backend]]) and the runtime stays swappable.
...
The move to a Rust backend ([[Architecture/ADR/010 Rust Backend]]) shifts that calculus — the Rust CRDT ecosystem is stronger — so a CRDT-based sync / real-time collaboration may be revisited. If adopted, it would supersede this decision.
