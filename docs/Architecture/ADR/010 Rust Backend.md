...
* Deno: a modern, secure-by-default TypeScript runtime; a sensible fit while the server still ran [[Plugs|plug]] logic and sandboxing mattered (pre-[[Architecture/ADR/007 Core Application Logic on the Client|v2]]).
...
* **Opens new doors.** A Rust backend makes a future CRDT-based sync engine / real-time collaboration feasible where the Go ecosystem made it impractical — see [[Architecture/ADR/002 Sync Engine]].
