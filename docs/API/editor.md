...
- `opts?` (`table`) — Optional `segment` (segment label), `phrase`, `dropdown` (dropdown value to select), and `focus` (`false` opens without taking focus). See [[API/view#view.open(name, opts?)]] for the full option semantics.
