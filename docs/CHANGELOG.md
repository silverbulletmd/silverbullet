...
* `@silverbulletmd/silverbullet/ui` grows the components the [[Navigator]] itself is built from, generalized for any plug: `Icon` (clone a pre-resolved icon node), `SegmentedControl` (a radiogroup that collapses to icons-only when it narrows), `ResizeHandle` (a draggable dock edge), `RowActions` (hover/selection-revealed row buttons) and `TreeView` (a drag-and-drop, keyboard-navigable tree) with its `tree_model.ts` data functions (`buildTree`, `computeTreeDisplay`, etc). No behavior change to the navigator itself. See [[Plugs/Development/Reference#Shared Components]].
