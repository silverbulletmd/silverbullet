...
* Beginnings of more solid [[Feature/Collaboration]] functionality, comprising of a slew of new improvements/features:
  ...
  * **HTML comments are now parsed as markdown** ([[Markdown/Comment]]): the body of a `<!-- ... -->` block is now parsed and rendered as ordinary markdown. This enables a communication channel (likely using [[Concept/Recipient|recipients]]) to communicate between team members.
    * `Comment: Add` on a list item now puts the comment under *that* item rather than after the whole list, and a comment region is drawn as a plain box instead of an indented, bar-marked gutter.
  * Beginning of [[Feature/Revisions]]: In three modes: **Managed** (SilverBullet commits your changes for you), **Unmanaged** (an existing git repository’s history is read, but SilverBullet never commits to it) or **Disabled**.
...
* **Completely revamped [[Feature/Navigator]] UX (includes a file tree!):** one configurable navigation UI that presents any object collection as a fuzzy-filterable list **or tree**, either as a modal or as a sidebar. A lot of views have been rebuilt on this. Oh yeah: ${widgets.commandButton("Navigate: Tree")} 🤯
* **[[Concept/Link|Wiki links]] now resolve by page name, not just by full path.** A `[[Note]]` link resolves to `some/folder/Note` when that name is unique in the space, matching how Obsidian resolves links, so an Obsidian-authored space works in SilverBullet without rewriting every link. See [[Architecture/ADR/011 Link Resolution by Name]] for reasoning. The `linkWriteFormat` option decides how SilverBullet writes the links _it_ generates (auto complete, rename backlink rewriting), it defaults to `full-path`, so generated links keep spelling out the whole path.
...
* The table-of-contents widget **is off by default**: replaced by two [[Feature/Navigator]] **outline** views over the same headers. The ToC widgets caused a lot of “content jiggling” on (first) load, which was quite distracting.
