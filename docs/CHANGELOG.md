...
* Completely revamped [[Navigator]] UX: one configurable navigation UI that presents any object collection as a fuzzy-filterable list **or tree**, either as a modal or as a sidebar. A lot of views have been rebuilt on this ([[Page Picker]], [[Command Palette]] etc.). I suppose I buried the lede here: one new instance of this is a ${widgets.commandButton("Navigate: Tree")} 🤯
...
* **HTML comments are markdown** ([[Markdown/Comment]]): the body of a `<!-- … -->` block is now parsed and rendered as ordinary markdown — paragraphs, lists, tasks, tables — with the markers hidden and the region tinted in [[Live Preview]]. Everything indexed inside a comment carries `inComment = true`, so a commented-out task **does** show up in `index.tasks()`; filter with `where not t.inComment` if you don't want it. A `space-lua` or `space-style` block inside a comment is not indexed and does not run — including on the pages read at boot ([[CONFIG]], libraries) — so commenting one out still disables it. Live evaluation in the open editor is untouched, though: `${...}` expressions and Lua-registered code widgets inside a comment still render. `Comment: Add` (`Ctrl-Alt-c`) inserts a comment after the current block, quoting any selection as a blockquote; `Comment: Selection` wraps the selected lines. The experimental comment grammar from earlier this cycle — `re:` anchors, `@addressee` routing, signatures, threads, the card widget and the `comment` object with `index.comments()` — is gone. A comment is labelled `comment` in its top right corner while its markers are hidden, and `Comment: Selection` inside a fenced code block comments in that block's own language (`//`, `#`, …) rather than wrapping it in an HTML comment, toggling back off when run again.
  * A `#tag` that occurs *only* inside comments on a page is flagged too, and left out of `#` completion, so commenting a block out no longer permanently registers its tags.
  * Because a comment body is now real markdown, smart quotes and slash commands work inside it like anywhere else. Both stay suppressed inside `<!--#lua …-->` / `<!--/lua-->` marker comments, which remain inert delimiters.
* Blockquote rendering fixes:
  ...
  * Quoted text can no longer render outside its own quote: a lazy-continuation line, or one whose marker was revealed by the cursor, used to draw to the left of the bar.
  * A multi-line [[Markdown/Admonition|admonition]] now tints all of its lines, instead of only the title line.
  * The quote bar is legible in dark mode again — both themes had accidentally been sharing a single colour.
