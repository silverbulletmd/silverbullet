...
---
A [[Space]] can be shared: several people with [[Dashboard|accounts]] on the same server, editing the same pages. SilverBullet supports this, but it does so in a very specific way.
...
There’s also a [[Guide/Working Together|guide on working together]].
...
# Conflicts
Sometimes both sides really did change the same words, and there is no sane way to merge them automatically. When that happens, the file is written with Git-style conflict markers. In practice, you never have to read that by hand. The editor [[Live Preview]]s each conflicted section as an _edit conflict_ widget showing both sides, as well as the original.
