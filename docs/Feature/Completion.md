...
Type `[[` to trigger page name completion. SilverBullet searches across all pages in your space and offers matching suggestions. Select one to insert a [[Concept/Link]] to that page. If no page with that name exists, a link is still created — navigating to it will create the page (these are tracked as [[Concept/Aspiring Page]]).
...
Inside a [[Concept/Frontmatter]] block, SilverBullet suggests attribute keys that are already used elsewhere in your space. This helps keep your metadata schema consistent.

# Slash commands
Type `/` at the beginning of a line (or after a space) to trigger [[Concept/Slash Command]] completion. Slash commands can insert templates, perform actions, or trigger custom behavior defined in [[Space Lua]].
