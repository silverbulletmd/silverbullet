...
The SilverBullet CLI is a companion command-line tool for interacting with a running SilverBullet instance from your terminal. It communicates with the server via the [[Feature/Runtime API]], letting you evaluate Lua expressions, run scripts, open an interactive REPL, tail logs, and more — without touching a browser.
...
# Commands
Note that the first call may take a few seconds, since the [[Feature/Runtime API]] will spin up a headless Chromium instance and need to perform an initial index for the first call.
...
* **Token** — sends an `Authorization: Bearer <token>` header. Use this with `SB_AUTH_TOKEN` on a [[Feature/Space Manager#Single-space mode|single-space]] server, or with a per-account [[Feature/Space Manager#API tokens|API token]] on an accounts-based server.
