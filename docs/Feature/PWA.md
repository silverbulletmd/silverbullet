...
# How offline works
SilverBullet's [[Feature/Sync]] engine keeps a full copy of your space in the browser's IndexedDB. When online, changes are continuously synced between the browser and the server. When offline, you work against the local copy. On reconnection, the sync engine reconciles any changes.

A service worker caches the application code itself, so even the SilverBullet UI loads without a network connection.

See also: [[Feature/Sync]], [[Feature/Collaboration]] (what happens to your offline edits when you reconnect), [[Architecture]]
