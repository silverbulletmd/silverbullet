...
---
How you authenticate depends on how the server is running (see [[Feature/Space Manager#Boot modes]]):
...
# API
For programmatic access via the [[HTTP API]], you can use bearer token authentication. In single-space mode, this token is configured with an environment variable, see [[Install/Configuration]]. In multi-space mode, new API tokens can be issued via the [[Feature/Space Manager]] UI.
