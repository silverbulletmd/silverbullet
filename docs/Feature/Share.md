...
**Note:** If you are interested in one-off ways to get content out, have a look at [[Concept/Export]].
...
* _Push_: produce content in SilverBullet, then push it to some external location, while being able to keep the external place up to date with changes, keeping SilverBullet as the source of truth. Example use cases:
  ...
  * [[Concept/Library]] you want to distribute to others
* _Pull_: pull in content from an external location and import it into your space and be able to keep pulling in new versions on demand. Use cases:
  * [[Concept/Library]] that you install
...
# Operation
SilverBullet has [[^Library/Std/Infrastructure/Share|infrastructural support]] to solve this problem in a general way. It leverages [[Concept/Frontmatter]], specifically the following three keys:

* `share.uri`: specifies the external location to push, pull or sync your content with, represented as a [[Concept/URI|URI]].
