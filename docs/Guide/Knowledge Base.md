...
> **note** Note
> This workflow works well with [[Feature/Journal]], where you link out to knowledge base topic pages from your journal pages.

# Create your first topic pages
Start with 3–4 “topic pages” on topics you know well: a programming language, a recipe, a book you've read. Open the [[Feature/Page Picker]] (`Cmd-k` / `Ctrl-k`), type a name, and press Enter.
...
Don’t worry about organizing pages into a folder structure, just put everything at the top level (see [[Guide/Best Practices]]). Just write and link. Structure emerges from connections.

# Discover connections via backlinks
Navigate to one of your linked pages (click a `[[link]]` or use the Page Picker). Scroll to the bottom and look for the [[Concept/Linked Mention]] section.
...
**[[Concept/Frontmatter]]** at the top of a page adds structured data and also be used as an alternative way to attach tags to a page:
```yaml
---
tags: book
...
Now this page is tagged `book` and has queryable `author` and `status` attributes. You can add tags to a page either with the `#book` syntax, or via a [[Concept/Frontmatter]] attribute.
...
See [[Concept/Template]] for more rendering options.

# Embed content across pages
Use [[Concept/Transclusion]] to pull content from one page into another. For instance on your [[Concept/Index Page]], transclude your “Currently Reading” page:
