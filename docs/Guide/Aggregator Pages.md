...
The query lives inside `${...}` and renders as a live widget in [[Feature/Live Preview]]. Because the prose body explains the pattern, the page retains value even outside SilverBullet.
...
2. **Write the aggregator page** — create a page (e.g. `Open Questions`) whose body is the SLIQ query above, substituting your tag. Use `index.contentPages("tag")` to filter out [[Concept/Meta Page|Meta Pages]]; use `index.objects("tag")` if you also want items from meta pages.
