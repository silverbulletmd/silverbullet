...
SilverBullet supports footnotes in two flavors: classic **reference-style** footnotes and **inline** footnotes. Both are rendered as a small widget in [[Features/Live Preview]] and show their content on hover.
...
# Editor behavior
* In [[Features/Live Preview]], a footnote reference renders as a compact `…` widget. Move the cursor into it to see and edit the raw `[^label]` source.
