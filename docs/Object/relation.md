...
* `toTag`: the target type — one of `page`, `anchor`, `document`, `url`, `identity`. Set for every resolved edge (absent for unresolved refs). `url` is a built-in target tag, like `document` (a real object type), `anchor` (a meta-tag) and `identity` (an [[Concepts/Identity]] name's namespaced `@name` identifier).
...
* `at-mention`: an [[Concepts/At-Mention|`@name` mention]] of an [[Concepts/Identity]]. `to` is always the name's `@name` identifier (`toTag: "identity"`); an identity is a name, so there is never a page behind it.
...
* #contact $pete-ref Pete [manager: "[[Zef Hemel]]"]
