...
* `toTag`: the target type — one of `page`, `anchor`, `document`, `url`, `identity`. Set for every resolved edge (absent for unresolved refs). `url` is a built-in target tag, like `document` (a real object type), `anchor` (a meta-tag) and `identity` (an [[Concept/Identity]] name's namespaced `@name` identifier).
...
* `at-mention`: an [[Concept/At-Mention|`@name` mention]] of an [[Concept/Identity]]. `to` is always the name's `@name` identifier (`toTag: "identity"`); an identity is a name, so there is never a page behind it.
