...
## Keyboard Shortcuts
Lists every [[Concepts/Command]] in the system and lets you rebind its shortcut. Features:
...
## Libraries
Manage installed [[Concepts/Library|Libraries]].

# How configuration is kept
The Configuration Manager writes your edits into a managed `space-lua` block inside your [[CONFIG]] page. Simple hand edits to the managed block generally survive a round-trip through the UI, but the authoritative format is whatever the Configuration Manager writes. Anything you add _outside_ that block is left alone.
