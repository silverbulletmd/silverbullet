...
# Commands
Note that the first call may take a few seconds, since the [[Features/Runtime API]] will spin up a headless Chromium instance and need to perform an initial index for the first call.
...
* **Token** — sends an `Authorization: Bearer <token>` header. Use this with `SB_AUTH_TOKEN` on a [[Features/Space Manager#Single-space mode|single-space]] server, or with a per-account [[Features/Space Manager#API tokens|API token]] on an accounts-based server.
