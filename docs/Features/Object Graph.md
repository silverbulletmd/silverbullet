...
* **Sidebar**
  ...
  * **Tags** — toggle visibility by user [[Concepts/Tag]]. Untagged objects share an `(untagged)` bucket.
...
* **Dashed outline** — dangling refs (referenced by some relation but not yet indexed — see [[Concepts/Aspiring Page]]).
