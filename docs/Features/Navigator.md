...
- client/navigator/*
---
The **navigator** is SilverBullet’s generalized navigation UI: it takes any collection of [[Object|objects]] and shows it as a fuzzy-filterable **list** or **tree**, either as a modal overlay or as a sidebar that stays open. The [[Features/Page Picker]], the [[Features/Command Palette]], and many others are all built on this abstraction.

# Built-in navigators
* [[Features/Page Picker]]: `Cmd-k`/`Ctrl-k`. The whole space as a modal list, most recently opened first.
* [[Features/Command Palette]]: `Cmd-/`/`Ctrl-/`. What you ran most recently first, each row showing its key binding.
...
* ${widgets.commandButton("Navigate: Linked Mentions")}: every other page that links to the one you're on, with a snippet of surrounding context. Docks at the bottom of the page by default, open. See [[Concepts/Linked Mention]].
* ${widgets.commandButton("Navigate: Linked Tasks")}: incomplete tasks on *other* pages that link to the one you're on. Docks at the top of the page by default, open. Tick a task's checkbox right in the widget and the new state is written back to the page the task lives on. See [[Concepts/Linked Task]].
...
* The **first character typed into an empty box can route**:
  * `^` narrows the page picker to [[Concepts/Meta Page|meta pages]].
...
`name`, `source` and `onSelect` are the only required keys, `command` registers a [[Concepts/Command]] that opens the view, and `key`/`mac` define a key binding for it. Open one from anywhere Lua runs with `view.open("tasks")`. (`navigator.*` is a permanent alias for `view.*` -- `navigator.define`, `navigator.open`, etc. still work.)
...
* `"page-top"` / `"page-bottom"` render as widgets built into the document itself, above and below the page content -- no filter box, just a title, a dock menu, a close button, and the view's own body. [[Concepts/Linked Mention|Linked Mentions]] and [[Concepts/Linked Task|Linked Tasks]] default to a page dock; [[#Built-in navigators|Table of Contents]] can be moved to one from its dock menu but opens as a modal by default.
