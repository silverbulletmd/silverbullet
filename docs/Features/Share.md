...
* _Push_: produce content in SilverBullet, then push it to some external location, while being able to keep the external place up to date with changes, keeping SilverBullet as the source of truth. Example use cases:
  ...
  * [[Concepts/Library]] you want to distribute to others
* _Pull_: pull in content from an external location and import it into your space and be able to keep pulling in new versions on demand. Use cases:
  * [[Concepts/Library]] that you install
...
# Operation
SilverBullet has [[^Library/Std/Infrastructure/Share|infrastructural support]] to solve this problem in a general way. It leverages [[Concepts/Frontmatter]], specifically the following three keys:

* `share.uri`: specifies the external location to push, pull or sync your content with, represented as a [[Concepts/URI|URI]].
