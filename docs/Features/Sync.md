...
## Status
If sync takes longer than a second, a black circle progress indicator will appear in the [[Concepts/Top Bar]].

## Conflicts
When multiple clients change the same file around the same time, the server merges them automatically where it safely can; when it can’t (both sides touched the same words), it writes the file with a resolvable in-editor conflict, and you will be notified in the UI when this happens. See [[Features/Collaboration]] for the full picture.
...
By default all your pages are synced, but [[Concepts/Document]] are not. They are fetched on demand. If you would also like to sync documents (so you can access them when offline), you can do so by setting the following in a Space Lua block in [[CONFIG]]:
