...
---
How you authenticate depends on how the server is running (see [[Features/Space Manager#Boot modes]]):
...
# Accounts
When the server runs in the default [[Features/Space Manager|multi-space]] mode, authentication is account-based:

* Every person has an **account** (username + password), plus an optional **Full name** and **Email** — set by an admin when creating the account or on its detail page, or by the account holder on their own **Profile** page. Both are used to attribute [[Features/Revisions#Automatic commits|revision-history commits]] and as the presence label other clients see for concurrent edits.
* Each [[Concepts/Space]] has an [[Features/Space Manager#Access|access level]] — `none`, `read`, or `write` — for visitors with no account, plus per-member `read`/`write` roles. Admins can reach every space and the admin UI.
...
# Single-space mode
[[Features/Space Manager#Single-space mode|Single-space mode]] serves one folder as one space, authenticated the classic way: a single set of credentials set via the `SB_USER` environment variable in `username:password` form.
...
# API
For programmatic access via the [[HTTP API]], you can use bearer token authentication. In single-space mode, this token is configured with an environment variable, see [[Install/Configuration]]. In multi-space mode, new API tokens can be issued via the [[Features/Space Manager]] UI.
