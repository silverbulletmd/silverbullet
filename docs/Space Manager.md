...
---
A single SilverBullet server can host any number of [[Space|spaces]] — each with its own URL, access rules, and configuration — managed through a web-based management UI called _Space Manager_.
...
There is no self-service signup. Admins create accounts. There is no password recovery either, an admin sets a new password from the _Users_ tab. Administrators can also provision [[Single Sign-On|SSO accounts]] alongside local accounts.
...
# Single-space mode
Single-space mode is the classic/soon legacy way to run a SilverBullet server: one folder, one space, configured entirely by environment variables. Pick it with `--single`, or simply by pointing the server at a folder that already has content (or by setting any legacy `SB_*` variable). See [[Authentication#Single-space mode]] for its authentication options and [[Install/Configuration]] for the full environment-variable surface. If the target folder doesn’t exist yet, the server creates it and serves an empty space.
...
* The runtime API (`runtimeApi`) has a server-wide switch under **Server**, a space setting, and an independent permission for each writer in the access grid. Existing writers default to enabled unless explicitly opted out. New spaces default on when Chrome is detected at startup and the server switch is enabled. Each user and space gets its own lazy Chrome process and temporary profile, with isolated cookies, storage, and logs. Removing Write or runtime access stops the affected browser. Startup logs report Chrome detection; `SB_CHROME_PATH` selects a non-standard executable. `SB_RUNTIME_API` applies only to single-instance mode.
