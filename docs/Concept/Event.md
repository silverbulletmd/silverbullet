...
---
SilverBullet has its own event bus that allows different parts of the system to communicate. Events are the foundation for much of SilverBullet's extensibility — features like [[Concept/Service]], [[Feature/Virtual Pages]], widgets, and custom indexing are all built on top of events.
...
* `editor:pageCreating`: A page is being created (can return content — used by [[Feature/Virtual Pages]])
...
* `editor:modeswitch`: Toggled between [[Feature/Vim Mode]] mode and normal mode
...
* `editor:complete`: Editor completion triggered — return completion results to extend [[Feature/Completion]]
