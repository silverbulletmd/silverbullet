...
In SilverBullet, there are multiple ways to attach [[Concept/Metadata]] to a page; frontmatter is the most popular one.

You create frontmatter by starting your markdown document with `---` followed by [[Concept/YAML]] encoded attributes and then ending with `---` again. Followed by the regular body of your document. This very page contains some frontmatter, click on it to see the underlying code.
...
* `tags` (`array of strings` or `string`): an alternative (and perhaps preferred) way to assign [[Concept/Tag]] to a page. There are various ways to define these:
  ```yaml
