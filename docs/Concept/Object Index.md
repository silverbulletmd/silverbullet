...
---
The Object Index stores [[Object|Objects]] found in your [[Concept/Space]].

You interact with it in a few ways:
* Indirectly via various other SilverBullet features such as [[Feature/Page Picker]], [[Feature/Meta Picker]], [[Concept/Linked Mention]] etc.
...
* `index.contentPages(tag?)` — returns only content pages, **excluding [[Concept/Meta Page|Meta Pages]]** (pages tagged `meta` or `meta/*`).
* `index.aspiringPages()` — returns all [[Concept/Aspiring Page]] (pages linked to but not yet created). Useful for finding broken or forward-references.
