---
references:
- client/data/datastore.ts
- client/data/object_index.ts
---
A personal _knowledge database_ is a place where you not only _write things down_, but where everything you write may eventually become structured, linked and queryable data.

In SilverBullet your notes are kept as a [[Concept/Space]] (folder) of [[Markdown]] [[Concept/Page|Pages]] on disk. As you write, SilverBullet continuously indexes your content — [[Concept/Page|pages]], [[Concept/Link|links]], [[Concept/Task|tasks]], list items and their [[Concept/Attribute|attributes]] — into queryable [[Object|Objects]]. You can then slice and reshape that data with [[Space Lua/Integrated Query|Queries]] (SLIQ) and surface the results right inside your notes.

It is SilverBullet’s take on **personal knowledge management** (PKM). From [Wikipedia](https://en.wikipedia.org/wiki/Personal_knowledge_management):

> Personal knowledge management (PKM) is a process of collecting information that a person uses to gather, classify, store, search, retrieve and share knowledge in their daily activities and the way in which these processes support work activities. It is a response to the idea that knowledge workers need to be responsible for their own growth and learning. It is a bottom-up approach to knowledge management.
