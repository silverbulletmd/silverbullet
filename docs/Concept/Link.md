...
* The "core" (After a possible `^` and before a `@` or `#`) of a string ref is a [[Name Rules|name]] or [[Concept/Path|path]].
...
A bare name that matches nothing at all is a dangling link and produces an [[Concept/Aspiring Page|aspiring page]] at the space root.

Name lookup also applies to [[Concept/Document|documents]] and [[Concept/Transclusion|transclusions]], so `![[diagram.png]]` finds `assets/diagram.png`. It does _not_ apply to relative [[#Internal links|Markdown links]], which keep their folder-relative meaning.
