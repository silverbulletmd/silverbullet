...
---
An **at-mention** is the `@name` span you type in text. It refers to an [[Concept/Identity]], and it has two directions: written plainly it *addresses* that identity, making it a [[Concept/Recipient]], or when used as a signature it *credits* that identity instead, see [[Concept/Authorship]].
...
**Any name is a valid mention.** Nothing needs declaring up front: mentioning @sales makes `sales` a known identity. See [[Concept/Identity]] for more info.