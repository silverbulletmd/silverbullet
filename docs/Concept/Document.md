...
tags: glossary
---
While SilverBullet is aimed at primarily text-based content, life can not fully be represented in text always. Therefore, SilverBullet supports documents. Documents, like [[Concept/Page]] ultimately are — once again — just files on disk. Using plugs, documents of specific file types can also be viewed and edited.
...
Using ${widgets.commandButton("Navigate: Document Picker")} you can open the document navigator, which will display all documents in your Silverbullet space. You can either:
  - Delete or rename documents for which no [[Feature/Document Editor]] is available (indicated by the grey hint)
  - Open and view documents for which a [[Feature/Document Editor]] is available (indicated by the blue hint)
